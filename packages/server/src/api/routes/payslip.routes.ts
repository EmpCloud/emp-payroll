...
router.get("/export/csv", authorize("hr_admin", "hr_manager"), wrap(async (req, res) => {
  const exportSvc = new ExportService();
  const csv = await exportSvc.exportPayslipsCSV(req.user!.orgId, req.query.runId as string);
...
router.post("/:id/dispute", wrap(async (req, res) => {
  const data = await svc.dispute(param(req, "id"), req.user!.userId, req.body.reason);
  res.json({ success: true, data });
}));

router.post("/:id/resolve", authorize("hr_admin", "hr_manager"), wrap(async (req, res) => {
