...
router.get("/computation/:empId", wrap(async (req, res) => {
  const data = await svc.getComputation(param(req, "empId"), req.query.fy as string);
  res.json({ success: true, data });
}));

router.post("/computation/:empId/compute", wrap(async (req, res) => {
  const data = await svc.computeTax(param(req, "empId"));
  res.json({ success: true, data });
}));

router.get("/declarations/:empId", wrap(async (req, res) => {
  const data = await svc.getDeclarations(param(req, "empId"), req.query.fy as string);
  res.json({ success: true, data });
}));

router.post("/declarations/:empId", validate(submitDeclarationSchema), wrap(async (req, res) => {
  const data = await svc.submitDeclarations(param(req, "empId"), req.body.financialYear, req.body.declarations);
  res.status(201).json({ success: true, data });
}));

router.put("/declarations/:empId/:declId", wrap(async (req, res) => {
  const data = await svc.updateDeclaration(param(req, "empId"), param(req, "declId"), req.body);
  res.json({ success: true, data });
}));

router.post("/declarations/:empId/approve", authorize("hr_admin", "hr_manager"), wrap(async (req, res) => {
  const data = await svc.approveDeclarations(param(req, "empId"), req.user!.userId);
  res.json({ success: true, data });
}));

router.get("/regime/:empId", wrap(async (req, res) => {
  const data = await svc.getRegime(param(req, "empId"));
  res.json({ success: true, data });
}));

router.put("/regime/:empId", wrap(async (req, res) => {
